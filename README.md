...
<head>
    ...
    <style>
        ...
        .answer-card {
            perspective: 1000px;
            cursor: pointer;
            /* Tighter height to fit on one screen */
            height: 3rem; /* Tighter on small screens */
        }
        ...
        .editor-input {
            @apply w-full bg-white/10 p-3 rounded-lg text-lg focus:outline-none focus:ring-2 focus:ring-yellow-400 text-white;
        }
        .editor-button {
                ...
                <div class="grid grid-cols-1 md:grid-cols-3 gap-3">
                    ...
                    <div class="flex flex-col gap-3">
                        ...
                        <div class="flex gap-2">
                            <input type="text" id="manual-answer-input" class="w-full bg-white/10 p-2 rounded-lg text-base focus:outline-none focus:ring-2 focus:ring-yellow-400" placeholder="Type answer...">
    ...
    <script type="module">
        ...
        function toggleMode() {
            if (currentMode === 'play') {
                ...
                // Show play mode only if the game has started
                if (!startMenuContainer.classList.contains('hidden')) {
                     // If we are still at the start menu, don't show the game board
                } else {
                    playModeContainer.classList.remove('hidden');
        ...
        function renderCustomizeUI() {
            ...
            addRoundButton.textContent = '✚ Add New Round';
            ...
            db.rounds.forEach((round, index) => {
                ...
                roundInfo.innerHTML = `
                    ...
                    <p class="text-gray-400">${(round.answers ? round.answers.length : 0)} answers</p>
                `;
        ...
        function renderRoundEditor(roundIndex) {
            ...
            backButton.textContent = '← Back to Round List';
            ...
            answersLabel.textContent = 'Answers (Top 8):';
            ...
            answersToRender.forEach((answer, answerIndex) => {
                ...
                pointsInput.className = 'editor-input w-24 edit-answer-points';
                answerRow.appendChild(pointsInput);
        ...
        function saveRound(roundIndex) {
            ...
            const newAnswers = [];
            const answerTextInputs = document.querySelectorAll('.edit-answer-text');
            const answerPointInputs = document.querySelectorAll('.edit-answer-points');
            
            for (let i = 0; i < answerTextInputs.length; i++) {
                const text = answerTextInputs[i].value;
                // Ensure points are parsed as an integer
                const points = parseInt(answerPointInputs[i].value, 10) || 0; 
                if (text.trim()) {
        ...
        function addRound() {
            db.rounds.push({
                ...
                answers: [
                    { text: "New Answer 1", points: 10 }
                ]
            });
            renderRoundEditor(db.rounds.length - 1); // Edit the new round
        }
        ...
        function addAnswer(roundIndex) {
            ...
            // Re-render the editor to show the new field, without saving yet
        ...
        function deleteAnswer(roundIndex, answerIndex) {
            ...
            // Re-render the editor immediately to reflect the deletion
            renderRoundEditor(roundIndex); 
        }
        ...
        startGameButton.addEventListener('click', startGame); // NEW
